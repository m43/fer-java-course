...
/**
 * Class models a tool that is used to draw circles. Drawn circles have only a
 * border and are not filled with any color. The color of the border is always
 * the foreground color. The class that models the geometric object is
 * {@link Circle}. The circle center is where the mouse made the first click and
 * the circle spans till the border that is determined by second mouse click.
 * Meanwhile while dragging the mouse pointer, a circle preview is drawn
 * simultaneously.
 * 
 * @author Frano Rajič
 */
...
//		}
//
//		Circle circleToPaint = new Circle(getFirst(), getSecond(), getForegroundColorProvider().getCurrentColor());
//		Painter.paint(g2d, circleToPaint);
//	}
//
//	@Override
//	void createNewObject() {
//		Circle newCircle = new Circle(getFirst(), getSecond(), getForegroundColorProvider().getCurrentColor());
...
//	}
	...
	public void paint(Graphics2D g2d) {
		...
		Circle circleToPaint = new Circle(first, second, getForegroundColorProvider().getCurrentColor());
		Painter.paint(g2d, circleToPaint);
	}

	@Override
	void createNewObject() {
		Circle newCircle = new Circle(first, second, getForegroundColorProvider().getCurrentColor());
		firstClick = false;
		second = null;
		getDrawingModel().add(newCircle);
	}
